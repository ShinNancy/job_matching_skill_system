...
`Java 21` · `Spring Boot 3` · `Next.js 14` · `FastAPI` · `PostgreSQL 16` · `Redis` · `Scrapy` · `Playwright` · `Docker Compose` · `Flyway` · `sentence-transformers` · `XGBoost`
...
| AI / NLP                             | Sentence Embeddings (SBERT), XGBoost, NLP Normalization    |
...
| C-03 | **Team size:** 3 người — thiết kế cần ưu tiên đơn giản, tránh over-engineering                                                 |
| C-04 | **Công nghệ:** Backend bắt buộc dùng Java Spring Boot; AI service bắt buộc dùng Python                                         |
...
> **TODO:** Các giá trị cụ thể bên dưới là _target ban đầu_ cho dự án cá nhân/portfolio. Cần validate và điều chỉnh dựa trên benchmark thực tế trong Phase 1.
...
> Đây là các ràng buộc thiết kế _hướng tới_, không phải yêu cầu bắt buộc trong MVP.
...
Lý do _không_ chọn full Microservice: độ phức tạp vận hành quá cao cho dự án cá nhân 1 người; Modular Monolith vẫn đảm bảo Separation of Concerns và dễ tách service về sau.
...
| **FastAPI AI Service**  | Python, FastAPI            | Embedding generation, match signal (Ranking), Skill Gap, Competition Score |
...
_Chịu trách nhiệm xác thực, phân quyền và quản lý tài khoản. Tách biệt với User Domain để tuân theo Separation of Concerns — thông tin đăng nhập không được mix với thông tin hồ sơ._
...
_Quản lý hồ sơ nghề nghiệp của người dùng — tách biệt với Authentication để mỗi domain có lifecycle riêng._
...
_Quản lý dữ liệu tuyển dụng đã được ETL và chuẩn hóa._
...
_Các bảng danh mục chuẩn hóa dùng chung toàn hệ thống. Đây là Master Data — không cho phép thay đổi tự do, phải qua Admin._
...
_Lưu kết quả AI và lịch sử matching để phục vụ analytics và future model training._
...
_Quản lý thông báo gửi qua Telegram._
...
| `notification_job` _(Optional)_ | Liên kết thông báo với danh sách job được gửi: `notification_id`, `job_id`                                                                            |

#### Domain 7: Data Pipeline

_Quản lý và audit quá trình thu thập, xử lý dữ liệu._
...
_"Python trả tín hiệu, Java ráp điểm"_
...
> **TODO:** Xem xét và finalize folder structure này trước Sprint 1. Cấu trúc dưới đây là _đề xuất ban đầu_ — chỉnh sửa nếu cần dựa trên kinh nghiệm implement.
...
_Tổ chức theo Clean Architecture + Domain (Bounded Context)_
...
### Phase 0 — Project Design _(Hiện tại)_
...
### Phase 1 — MVP Core _(Backend + Data Pipeline)_
...
### Phase 2 — AI MVP _(FastAPI + Vector Search)_
